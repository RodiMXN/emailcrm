...
const LEADS_DUE_TODAY_VIEW_UNIVERSAL_IDENTIFIER =
  STANDARD_OBJECTS.person.views.leadsDueToday.universalIdentifier;

const LEADS_DUE_TODAY_VIEW_FIELD_UNIVERSAL_IDENTIFIERS = [
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFields.currentState
    .universalIdentifier,
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFields.nextActionType
    .universalIdentifier,
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFields.nextActionDate
    .universalIdentifier,
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFields.isSuppressed
    .universalIdentifier,
] as const;

const LEADS_DUE_TODAY_VIEW_FILTER_UNIVERSAL_IDENTIFIERS = [
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFilters
    .nextActionDateIsNotEmpty.universalIdentifier,
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFilters.nextActionDateIsInPast
    .universalIdentifier,
  STANDARD_OBJECTS.person.views.leadsDueToday.viewFilters.isSuppressedIsFalse
    .universalIdentifier,
] as const;

@RegisteredWorkspaceCommand('2.1.0', 1780000007000)
@Command({
  name: 'upgrade:2-1:backfill-person-leads-due-today-view',
  description:
    'Backfill person leadsDueToday standard view, its approved view fields, and its approved filters',
})
export class BackfillPersonLeadsDueTodayViewCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    const viewToCreate = this.computeMissingLeadsDueTodayView({
      existingFlatViewMaps,
      standardAllFlatEntityMaps,
    });

    const viewFieldsToCreate = this.computeMissingLeadsDueTodayViewFields({
      existingFlatViewFieldMaps,
      standardAllFlatEntityMaps,
    });

    const viewFiltersToCreate = this.computeMissingLeadsDueTodayViewFilters({
    ...
    if (
      ...
      this.logger.log(
        `Nothing to backfill for person leadsDueToday view in workspace ${workspaceId}`,
      );
    ...
    this.logger.log(
      `${isDryRun ? '[DRY RUN] ' : ''}Backfilling person leadsDueToday view metadata in workspace ${workspaceId}: creating ${isDefined(viewToCreate) ? 1 : 0} views, ${viewFieldsToCreate.length} view fields, ${viewFiltersToCreate.length} view filters`,
    );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to backfill person leadsDueToday metadata for workspace ${workspaceId}:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );
    ...
    this.logger.log(
      `Successfully backfilled person leadsDueToday metadata for workspace ${workspaceId}`,
    );
  }

  private computeMissingLeadsDueTodayView({
    existingFlatViewMaps,
    standardAllFlatEntityMaps,
  }: {
    ...
    const existingView =
      existingFlatViewMaps.byUniversalIdentifier[
        LEADS_DUE_TODAY_VIEW_UNIVERSAL_IDENTIFIER
      ];
    ...
    if (!isDefined(standardView)) {
      this.logger.warn(
        `Standard view not found for universalIdentifier ${LEADS_DUE_TODAY_VIEW_UNIVERSAL_IDENTIFIER}, skipping`,
      );
  ...
  private computeMissingLeadsDueTodayViewFields({
    existingFlatViewFieldMaps,
    standardAllFlatEntityMaps,
  }: {
    ...
    return LEADS_DUE_TODAY_VIEW_FIELD_UNIVERSAL_IDENTIFIERS.flatMap(
      (universalIdentifier) => {
  ...
  private computeMissingLeadsDueTodayViewFilters({
    existingFlatViewFilterMaps,
    standardAllFlatEntityMaps,
  }: {
    ...
    return LEADS_DUE_TODAY_VIEW_FILTER_UNIVERSAL_IDENTIFIERS.flatMap(
      (universalIdentifier) => {
